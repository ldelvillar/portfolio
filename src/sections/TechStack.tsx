...
export default function TechStack() {
  return (
    <section
      id="techstack"
      className="scroll-m-20 mt-36 w-full mx-auto container lg:max-w-4xl md:max-w-2xl pb-10"
    >
      <div className="mb-14">
        ...
        <div className="flex flex-col md:grid md:grid-cols-2 gap-5">
          {FIELDS.map(field => {
            const displayedTechnology = field.technologies.map(technology => (
              <div
                className="flex flex-col justify-center items-center text-gray-800 dark:text-gray-200"
                key={nanoid()}
              >
            ...
            return (
              <Atropos activeOffset={40} shadowScale={1.05} key={nanoid()}>
                <div className="h-[100%] flex flex-col items-center bg-white dark:bg-primary border dark:border-gray-800 rounded-2xl">
                  <h2 className="text-3xl font-bold text-gray-100 bg-gradient-to-t from-[#fffc5f] to-[#c9b500] dark:from-[#fffec9] dark:to-[#efeb00] bg-clip-text text-transparent mt-3">
                    {field.name}
                  </h2>
